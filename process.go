...
func killHarnessLocked() {
	...
	for _, pid := range findPidsOnPort(port) {
		if isProcessAlive(pid) {
			if isDshProcess(pid) {
				LogInfo("[进程] 终止端口 %d 残留进程 (PID=%d)", port, pid)
				killProcessTree(pid)
				_ = killProcessGroup(pid)
			} else {
				LogWarning("[进程] 端口 %d 被非托管外部进程占用 (PID=%d)，跳过清理", port, pid)
			}
		}
	}
	...
	if curStatus == StatusRunning || curStatus == StatusStarting {
		...
		if adopted := findDshPidOnPort(port); adopted > 0 {
			...
			if adopted != currentPid {
				LogInfo("[进程] 接管端口 %d 活跃子进程 (PID=%d)", port, adopted)
				procMu.Lock()
		...
		LogWarning("[进程] 主进程已终止 (PID=%d) 且端口 %d 无响应，执行清理", currentPid, port)
		procMu.Lock()
